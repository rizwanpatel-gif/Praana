...
function generateVitals(pid: string, steps: number, stepSecs: number): Vitals[] {
  ...
  if (!cfg) return [];
  const result: Vitals[] = [];
  // Use a deterministic-ish seed per patient so data looks stable across re-renders
  const seed = pid.charCodeAt(1) || 1;
  for (let i = steps; i >= 0; i--) {
    const phase = ((steps - i) / steps) * 2 * Math.PI;
    const s = Math.sin(phase);
    const r = Math.sin((steps - i) * seed * 9301 + 49297) * 0.5;
    result.push({
      id: `dv-${pid}-${i}`,
      patient_id: pid,
      org_id: 'demo',
      recorded_by: 'Demo',
      heart_rate:       clamp(Math.round(cfg.hr  + s*cfg.hrV/2   + r*cfg.hrV),   40,  200),
      systolic_bp:      clamp(Math.round(cfg.bp  + s*cfg.bpV/2   + r*cfg.bpV),   70,  220),
      diastolic_bp:     clamp(Math.round(cfg.dbp + s*cfg.bpV/3   + r*cfg.bpV/2), 40,  130),
      temperature:      clamp(Math.round((cfg.temp+s*cfg.tempV/2 + r*cfg.tempV)*10)/10, 35, 42),
      spo2:             clamp(Math.round(cfg.spo2 + s*cfg.spo2V/2+ r*cfg.spo2V), 70,  100),
      respiratory_rate: clamp(Math.round(cfg.rr  + s*cfg.rrV/2   + r*cfg.rrV),   8,   40),
      recorded_at: ts(i * stepSecs),
      notes: '',
    });
  }
  return result;
...
function makeAlerts(patients: Patient[]): Alert[] {
  const alerts: Alert[] = [];
  for (const p of patients.filter(x => x.status === 'critical')) {
    const cfg = VCFG[p.id];
    alerts.push({
      id: `da-${p.id}-hr`, org_id:'demo', patient_id:p.id, patient_name:p.name,
      vital_type:'heart_rate', value:cfg.hr+cfg.hrV, threshold:100,
      severity:'critical', message:`${p.name}: heart_rate ${cfg.hr+cfg.hrV} exceeds 100.0`,
    ...
    alerts.push({
      ...
      severity:'critical', message:`${p.name}: spo2 ${cfg.spo2} below 92.0`,
...
function makeAlertHistory(patients: Patient[]): Alert[] {
  return patients.map((p, i) => {
    ...
    return {
      ...
      acknowledged:true, acknowledged_at:ts((i+2)*3600-1800), created_at:ts((i+2)*3600),
    };
  });
}
...
export class DemoService {
  ...
  patient(id: string): Observable<ApiResponse<{ patient: Patient; latest_vitals: Vitals | null }>> {
    ...
    return of({ success: true, data: { patient: p, latest_vitals: vitals[vitals.length - 1] ?? null } });
  }
  ...
  vitalsHistory(pid: string, range: string): Observable<ApiResponse<Vitals[]>> {
    ...
    const [steps, stepSecs] = cfg[range] ?? [48, 30*60];
  ...
  dashboardOverview(): Observable<ApiResponse<DashboardOverview>> {
    ...
    const summaries = active.map(p => {
      const v = generateVitals(p.id, 48, 30*60);
      return { patient: p, latest_vitals: v[v.length - 1], alert_count: p.status === 'critical' ? 2 : 0 };
    });
  ...
  shiftSummary(): Observable<ApiResponse<ShiftSummary>> {
    const n = ts();
    return of({ success: true, data: {
      shift_start: n - 6*3600, shift_end: n + 2*3600,
      vitals_recorded: 47, alerts_triggered: 6, alerts_acknowledged: 4, patients_checked: 8,
    }});
  }
  ...
  alertHistory(): Observable<ApiResponse<Alert[]>> {
    ...
    return of({ success: true, data: [...makeAlerts(p), ...makeAlertHistory(p)] });
  }
